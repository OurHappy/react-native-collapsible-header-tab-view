...
import type { GesturePanContext } from './types';
...
export const toEndSlide = ({
  ...
  'worklet';
  ctx.starty = 0;
  transValue.value = withDecay(
    { velocity: velocityY, deceleration: 0.997, clamp: [0, Number.MAX_VALUE] },
    (_finished) => {
...
export const animateToRefresh = ({
  ...
  'worklet';
  if (isToRefresh === true && isRefreshing.value === true) {
    return;
  }
  if (
    isToRefresh === false &&
  ...
  transRefreshing.value = withTiming(destPoi, undefined, (_finished) => {
    isRefreshingWithAnimation.value = isToRefresh;
  });
};
...
export const snapAfterGlideOver = ({
  ...
  if (shareAnimatedValue.value > calcH) {
    return;
  }
  const poi = shareAnimatedValue.value < calcH * 0.5 ? 0 : calcH;
  if (shareAnimatedValue.value === poi) {
    return;
  }
