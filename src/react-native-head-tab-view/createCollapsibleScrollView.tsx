...
import type { ScrollableView } from './types';
import {
  Platform,
  StyleSheet,
  ScrollView,
...
import type { NormalSceneProps, HPageViewProps } from './types';
import {
  useSceneContext,
  ...
  const onScrollAnimateEvent = useAnimatedScrollHandler(
    {
      onScroll: (event, _ctx) => {
        ...
        if (isRefreshingWithAnimation.value || isRefreshing.value) {
          return;
        }
        tryToSnap();
        moveY =
