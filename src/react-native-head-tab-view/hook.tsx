...
import type { ForwardRefType, updateSceneInfoType } from './types';
...
export const useSyncInitialPosition = (_scrollView: any) => {
  ...
  const syncInitialPosition = useCallback(
    ...
    [headerHeight, frozeTop, _scrollView, opacityValue]
  );
...
export const useSceneInfo = (curIndexValue: number) => {
  ...
  const [sceneScrollEnabledValue, setSceneScrollEnabledValue] = useState<{
    [index: number]: Animated.SharedValue<boolean>;
  }>({});
  ...
  const [sceneRefreshTrans, setSceneRefreshTrans] = useState<{
    [index: number]: Animated.SharedValue<number>;
  }>({});
  const [sceneIsRefreshing, setSceneIsRefreshing] = useState<{
    [index: number]: Animated.SharedValue<boolean>;
  }>({});
  const [sceneIsRefreshingWithAnimation, setSceneIsRefreshingWithAnimation] =
    useState<{ [index: number]: Animated.SharedValue<boolean> }>({});
  const [sceneIsDragging, setSceneIsDragging] = useState<{
    [index: number]: Animated.SharedValue<boolean>;
  }>({});
  const [sceneIsLosingMomentum, setSceneIsScrolling] = useState<{
    [index: number]: Animated.SharedValue<boolean>;
  }>({});
  const [sceneCanPullRefresh, setSceneCanPullRefresh] = useState<{
    [index: number]: boolean;
  }>({});
  const [sceneRefreshCallBack, setSceneRefreshCallBack] = useState<{
    [index: number]: (isToRefresh: boolean) => void;
  }>({});
  ...
  const updateSceneInfo = useCallback(
    ({
      ...
      if (
        isLosingMomentum &&
        sceneIsLosingMomentum[index] !== isLosingMomentum
      ) {
      ...
      if (
        scrollEnabledValue &&
        sceneScrollEnabledValue[index] !== scrollEnabledValue
      ) {
      ...
      if (
        onRefreshStatusCallback &&
        sceneRefreshCallBack[index] !== onRefreshStatusCallback
      ) {
...
export const useVerifyProps = ({
  ...
  if (contentContainerStyle) {
    ...
    if (paddingTop !== undefined || minHeight !== undefined) {
      console.warn(
        'Please do not assign paddingTop and minHeight to contentContainerStyle'
      );
    }
