...
export default class MainScreen extends React.PureComponent<Props, State> {
  ...
  _renderItem = ({
    ...
    item: {title: string; description: string; page: string};
  }) => {
    return (
      <TouchableOpacity
  ...
  render() {
    return (
      <SafeAreaView style={{flex: 1}}>
        {staticData.homeConfig.map((item, index) => (
          <SelectView
            key={'SELECT_' + index}
              ...
              <TouchableOpacity
                ...
                <View
                  style={{
                    ...
                    height: 10,
                    borderWidth: 1,
                    borderColor: isSelect ? '#FFD321' : '#888',
                    backgroundColor: isSelect ? '#FFD321' : 'transparent',
                  }}
                />
